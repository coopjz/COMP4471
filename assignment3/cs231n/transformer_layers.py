...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pow_term = torch.pow(torch.full((length,), 10000), -
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # split your shape from (N, T, E) into (N, T, H, E/H), where H is the number of heads.
        query = self.query(query).view(N, -1, self.n_head,
                                       self.head_dim).transpose(1, 2)
        key = self.key(key).view(N, -1, self.n_head,
                                 self.head_dim).transpose(1, 2)
        value = self.value(value).view(N, -1, self.n_head,
                                       self.head_dim).transpose(1, 2)
        ...
        if attn_mask is not None:
            weights = weights.masked_fill(attn_mask.eq(0), float('-inf'))

        # softmax
        weights = F.softmax(weights, dim=-1)
        weights = self.attn_drop(weights)

        # (N, H, T, T) by (N, H, T, E/H) to yield a shape (N, H, T, E/H)]
