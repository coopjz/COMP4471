...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i / torch.linalg.norm(z_i)
                        ) @ (z_j / torch.linalg.norm(z_j))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_1_d = torch.tensor(
            [sim(z_k, out[i]) / tau for i in range(2*N) if i != k]).exp().sum()
        total_loss += -torch.log(l_1_u / l_1_d)

        # l(k+N, k)
        l_2_u = torch.exp(sim(z_k_N, z_k) / tau)
        l_2_d = torch.tensor(
            [sim(z_k_N, out[i]) / tau for i in range(2*N) if i != k+N]).exp().sum()
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    out_left = out_left / torch.linalg.norm(out_left, dim=1).view(-1, 1)
    out_right = out_right / torch.linalg.norm(out_right, dim=1).view(-1, 1)
    pos_pairs = (out_left * out_right).sum(dim=1).view(-1, 1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    similarity = torch.cat((sim_positive_pairs(
        out_left, out_right), sim_positive_pairs(out_right, out_left)))
    pass
